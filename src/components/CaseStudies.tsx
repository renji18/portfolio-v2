...
const CaseStudies = () => {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm lg:text-base text-center md:w-[480px] leading-[1.7] px-[15px]">
      ...
      <div className="pt-[80px] grid md:grid-cols-2 px-4 lg:px-20 gap-6">
        {caseStudies?.map((c) => (
          <div
            key={c.title}
            className="rounded-3xl bg-lightWhite text-darkBlack p-4 lg:p-6 border border-white/10 hover:-translate-y-1 transition-all flex flex-col justify-between gap-3 lg:gap-4"
          >
            <div>
            ...
            <div>
              ...
              <div className="flex gap-3 mt-4 lg:mt-6">
                {c.metrics.map((m) => (
                  <span
                    key={m}
                    className="px-3 py-1 rounded-full bg-darkBlack text-myBlue text-xs lg:text-sm"
                  >
                    {m}
                  </span>
                ))}
            ...
            <button
              className="flex items-center gap-2 text-myBlue font-medium lg:font-semibold"
