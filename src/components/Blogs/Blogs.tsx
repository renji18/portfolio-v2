...
import type { BlogInterface } from "../../interface/portfolio.interface";
...
const Blogs = () => {
  ...
  const blogs = useMemo(
    () => portfolio?.blogs?.slice()?.reverse(),
    [portfolio?.blogs],
  );

  const [visibleBlogs, setVisibleBlogs] = useState<Array<BlogInterface>>([]);
  ...
  return (
    ...
    >
      ...
      {blogs && visibleBlogs.length < blogs?.length && (
